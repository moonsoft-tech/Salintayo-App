...
import { IonContent,
  IonFooter, IonIcon, IonPage, IonRouterLink, useIonViewDidEnter, useIonViewWillEnter } from '@ionic/react';
import {
  bookOutline,
  documentTextOutline,
  homeOutline,
...
/**
 * Firestore's setDoc()/updateDoc() reject any field whose value is literally
 * `undefined` — the key must be omitted entirely (or set to `null`) instead.
 * Several optional ChatMessage fields (audioUrl, audioDuration, imageUrl,
 * translationMode) are frequently `undefined` for text-only or
 * transcript-only messages, so every cloud write must be sanitized before
 * being handed to Firestore. Centralizing this here — rather than fixing it
...
const ChatPage: React.FC = () => {
  ...
  // Firestore round trips (migration + existence check) are still in flight.
  // Without this, a late `setCurrentChatId(preferred)` from the resolve
  // effect can silently revert the user's manual selection right after they
  // make it, which looks like "selecting a conversation does nothing."
  const manualChatSelectionRef = useRef(false);
  ...
  // shrinks, which drags every `position: fixed; bottom: 0` element (including
  // the bottom nav) up along with it. We detect the keyboard by comparing the
  // live viewport height against the height captured on mount, then flag it
  // via `isKeyboardOpen` so the nav can be hidden (it would otherwise float
  // above the input box) while the input bar docks flush against the keyboard.
  const [isKeyboardOpen, setIsKeyboardOpen] = useState(false);
  useEffect(() => {
    const baselineHeight = window.visualViewport?.height ?? window.innerHeight;
    const KEYBOARD_THRESHOLD_PX = 120; // ignore small chrome/URL-bar height changes

    const checkKeyboard = () => {
      const currentHeight = window.visualViewport?.height ?? window.innerHeight;
      const heightDelta = baselineHeight - currentHeight;
      setIsKeyboardOpen(heightDelta > KEYBOARD_THRESHOLD_PX);
    };
  ...
  const ensureChatThread = useCallback(async (): Promise<string | null> => {
    const uid = user?.uid;
    if (!uid) return null;

    const existing = currentChatIdRef.current;
    ...
    try {
      ...
      console.error('ensureChatThread', err);
      return null;
    }
  }, [user?.uid]);
  ...
  useEffect(() => {
    ...
    // A fresh resolution cycle is starting for this user — clear any stale
    // "manual selection" flag from a previous mount/user so this cycle is
    // allowed to run, unless the user acts again before it finishes.
    manualChatSelectionRef.current = false;
    setMessages([]);
    let cancelled = false;
    void (async () => {
      try {
      ...
      if (cancelled || manualChatSelectionRef.current) return;
      ...
      try {
        try {
          sessionStorage.removeItem(activeChatIdStorageKey(user.uid));
        } catch {
          /* ignore */
        }
        if (!cancelled && !manualChatSelectionRef.current) {
          setCurrentChatId(null);
        }
      } catch (e) {
        console.error('Could not initialize chat thread:', e);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const startNewChat = () => {
    // This is an explicit user action — do not let the (possibly still
    // in-flight) initial thread-resolution effect override it afterwards.
    manualChatSelectionRef.current = true;
  ...
  // Opens a conversation selected from the history drawer.
  // For signed-in users this only needs to update `currentChatId` — the
  // `subscribeChatMessages` effect above reacts to that change and loads
  // the thread's messages live, so we don't duplicate that work here with
  // a second one-time fetch (that duplicate fetch was a source of races).
  const handleSelectSession = useCallback(
    (session: ChatSession) => {
      // Mark this as a deliberate selection so the initial thread-resolution
      // effect (which may still be resolving in the background) won't
      // silently revert it once its own async chain completes.
      manualChatSelectionRef.current = true;
  ...
  return (
    <IonPage>
      <IonContent fullscreen className="chat-content" ref={chatContentRef}>
        <div className={`chat-page ${isKeyboardOpen ? 'chat-page--keyboard-open' : ''}`}>
      ...
      <IonFooter className={`chat-footer ion-no-border ${isKeyboardOpen ? 'chat-footer--keyboard-open' : ''}`}>
        <nav className="chat-nav" aria-label="Main">
          <IonRouterLink routerLink="/home" routerDirection="root" className="chat-nav__item">
            <IonIcon icon={homeOutline} className="chat-nav__icon" />
            <span className="chat-nav__label">Home</span>
          </IonRouterLink>
          <IonRouterLink routerLink="/chat" routerDirection="root" className={`chat-nav__item ${isChat ? 'chat-nav__item--active' : ''}`}>
            <IonIcon icon={chatbubbleOutline} className="chat-nav__icon" />
            <span className="chat-nav__label">Chat</span>
          </IonRouterLink>
          <IonRouterLink routerLink="/profile" routerDirection="root" className="chat-nav__item">
